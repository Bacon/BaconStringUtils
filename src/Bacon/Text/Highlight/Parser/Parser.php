...
class Parser
{
     ...
     * @return array
     */
    public function parseString($source, $language)
    {
        $this->data = array();
    }
    ...
    {
        if ($insertNewLine) {
            $this->whitespaceBuffer .= $this->getNewLine();
        }
    }
    ...
    protected function processRootState()
    {
        ...
        $this->generator->openTag(States::STANDARD);
        ...
        $this->appendString($this->getNewLine());
    }
    ...
    protected function processKeywordState($state)
    {
        ...
        do {
            $this->appendMaskedToken(true, $newState !== States::_WS, ($this->currentSyntax->shouldIgnoreCase() ? '' : '')); // @todo
            $newState = $this->getCurrentState($state);
